...
@Injectable()
export class AppointmentEventPublisherService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(AppointmentEventPublisherService.name);
  ...
  private async connect(): Promise<void> {
    ...
    try {
      ...
      this.logger.log('Successfully connected to RabbitMQ - audit events will be published');
      ...
      this.logger.error(`AUDIT EVENTS DISABLED: Failed to connect to RabbitMQ at ${rabbitmqUrl}: ${(error as Error).message}`);
      this.isConnected = false;
      this.scheduleReconnect();
    }
  }
